...
def capture_image():
    """Capture image from webcam with robust error handling"""
    cap = None
    try:
        ...
        for _ in range(5):  # Try reading multiple frames to stabilize
            ret, frame = cap.read()
            if ret:
                break
            sleep(0.1)
        ...
        img = cv2.resize(img, (1024, 768))  # Standardize size
...
if st.button("✨ Generate Magic ✨", use_container_width=True, type="primary"):
    if img is not None and validate_image(img):
        with st.spinner("Working magic with your image..."):
            ...
            st.image(img, use_container_width=True)  # Fixed: Replaced use_column_width
            ...
            if processed_img:
                ...
                with tab1:
                    ...
                    st.markdown("### 📝 Extracted Image Details")
